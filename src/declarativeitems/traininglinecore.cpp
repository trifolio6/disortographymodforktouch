...
int TrainingLineCore::rabbitSpeed() const
{
    return Preferences::rabbitStrokesPerMinute();
}
...
QDeclarativeItem* TrainingLineCore::cursorRabbitItem() const
{
    return m_cursorRabbitItem;
}
...
void TrainingLineCore::setCursorRabbitItem(QDeclarativeItem* cursorPosition)
{
    if (cursorPosition != m_cursorItem)
    {
        m_cursorRabbitItem = cursorPosition;
        emit cursorRabbitItemChanged();
    }
}
